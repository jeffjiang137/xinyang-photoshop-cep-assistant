(function () {
  ...
  function runEmbed(multiple) {
    var files = pickImages(multiple);
    if (!files.length) { status("未选择图片"); return; }
    runHost("toolsImportImages", {
      files: files,
      mode: "smart",
      layout: "overlay",
      gap: 0,
      fit: "original"
    }, "正在嵌入 " + files.length + " 张图片…", function (result) {
      return "已嵌入 " + (result.imported || files.length) + " 张图片";
    }).catch(function () {});
  }
  ...
  function openDetail(detail) {
    ...
    var active = one('#common-tools-parameter-drawer [data-tool-detail-view="' + detail + '"]');
    if (active && active.scrollIntoView) active.scrollIntoView(false);
    status("已展开参数设置");
  }
  ...
  function runAction(action) {
    ...
    else if (action === "import-images") openDetail("import-images");
    ...
    else if (action === "custom-transform") openDetail("quick-transform");
    else if (action === "export-image") openDetail("batch-export");
  ...
  function bind() {
    all("[data-common-action]").forEach(function (button) {
      button.addEventListener("click", function () { runAction(button.getAttribute("data-common-action")); });
    });
    all("[data-common-more]").forEach(function (button) {
      button.addEventListener("click", function (event) {
        event.stopPropagation();
        openDetail(button.getAttribute("data-common-more"));
      });
    });
